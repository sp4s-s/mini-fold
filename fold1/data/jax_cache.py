...
def embed_protein(sequence, esm_model, alphabet, device):
    ...
    return jnp.array(embedding)  # convert to JAX array
...
def embed_ligand(smiles, tokenizer, model, device):
    ...
    return jnp.array(embeddings)  # convert to JAX array
...
def cache_embeddings(df, esm_model, alphabet, esm_device, tokenizer, smiles_model, smiles_device):
    ...
    for idx, row in enumerate(df.iter_rows(named=True)):
        ...
        try:
            ...
            np.savez(filename, protein=np.array(p_emb), ligand=np.array(l_emb), label=affinity)
            if idx % 100 == 0:
...
def load_cached_embedding(filepath):
    ...
    protein = jnp.array(data['protein'])
    ligand = jnp.array(data['ligand'])
    label = data['label'].item()  # scalar affinity
